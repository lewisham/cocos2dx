...
DBCCArmatureNode* DBCCFactory::buildArmatureNode(const std::string &armatureName, const std::string &dragonBonesName,WorldClock* clock) const
{
    auto armature = buildArmature(armatureName, dragonBonesName);
    return DBCCArmatureNode::createWithWorldClock(armature,clock);
}
...
DragonBonesData* DBCCFactory::getDragonBonesDataFromFile(const std::string &dragonBonesFilePath){
...
DragonBonesData* DBCCFactory::loadDragonBonesData(const std::string &dragonBonesFilePath, const std::string &name)
{
    ...
    {
        if (existDragonBonesData->addDBFile(dragonBonesFilePath)){
            DragonBonesData *dragonBonesData = getDragonBonesDataFromFile(dragonBonesFilePath);
            if (dragonBonesData){
                existDragonBonesData->addArmatureData(dragonBonesData);
                delete dragonBonesData;
            }
        }
    ...
    if (dragonBonesData)
    {
		...
		{
			//cocos2d::log("dragonBonesData %s", dragonBonesData->armatureDataList[i]->name);
		}
...
//    dragonBones::XMLDataParser parser;
//    DragonBonesData *dragonBonesData = parser.parseDragonBonesData(doc.RootElement(), scale);
//    dragonBonesData->addDBFile(dragonBonesFilePath);
//    addDragonBonesData(dragonBonesData, name);
//    return dragonBonesData;
}
...
ITextureAtlas* DBCCFactory::loadTextureAtlasFromPlist(const std::string &plist,const std::string &name){
    ITextureAtlas *existTextureAtlas = getTextureAtlas(name);
    
    if (existTextureAtlas)
    {
        DBCCTextureAtlas *textureAtlas = static_cast<DBCCTextureAtlas*>(existTextureAtlas);
        if (textureAtlas->addpListFile(plist)){
            refreshTextureAtlasTexture(name.empty() ? textureAtlas->name : name);
        }
        return textureAtlas;
    }
    
    DBCCTextureAtlas *textureAtlas = new DBCCTextureAtlas();
    textureAtlas->addpListFile(plist);
    textureAtlas->name = name;
...
// 删除一个DB 数据
const std::string DBCCFactory::removeDBDataByKey(const std::string &name){
...
#else
    DBCCTextureAtlas *dbccTextureAtlas = (DBCCTextureAtlas*)(textureAtlas);
    std::string spriteFrameName = displayData->name;
    cocos2d::SpriteFrame* sf = cocos2d::SpriteFrameCache::getInstance()->getSpriteFrameByName(spriteFrameName);
    if (!sf){
        dbccTextureAtlas->reloadTexture();
        sf = cocos2d::SpriteFrameCache::getInstance()->getSpriteFrameByName(spriteFrameName);
    }
    
    cocos2d::Node *display = cocos2d::Sprite::createWithSpriteFrame(sf);
    cocos2d::Size originSize = display->getContentSize();
    display->setCascadeColorEnabled(true);
    display->setCascadeOpacityEnabled(true);
    display->retain();
    float pivotX = 0.f;
