...
void DBCCArmature::setAllGLParamState(cocos2d::GLProgramState *state){
    DBCCUtils::setAllGLParamState(this, state);
}
...
void DBCCArmature::registerSoundEventHandler(cocos2d::LUA_FUNCTION func){
    unregisterSoundEventHandler();
    _soundEventHandler = func;
    ...
    auto f = [this](cocos2d::EventCustom *event)
    {
        ...
        DBCCLuaUtils::pushEventData(eventData, _armatureNode);
        auto stack = cocos2d::LuaEngine::getInstance()->getLuaStack();
        stack->executeFunctionByHandler(_soundEventHandler, 1);
    };
    
    dispatcher->addCustomEventListener(EventData::SOUND, f);
}
...
void DBCCArmature::unregisterSoundEventHandler(){
    if (_soundEventHandler != 0)
    {
        auto dispatcher = getCCEventDispatcher();
        dispatcher->removeCustomEventListeners(EventData::SOUND);
        cocos2d::LuaEngine::getInstance()->removeScriptHandler(_soundEventHandler);
        _soundEventHandler = 0;
    }
}
