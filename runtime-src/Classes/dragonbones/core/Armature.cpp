...
#else
    if (!frame->sound.empty() && _eventDispatcher->hasEvent(EventData::EventType::ANIMATION_FRAME_EVENT))
    {
        EventData *eventData = EventData::borrowObject(EventData::EventType::SOUND);
        eventData->armature = this;
        eventData->animationState = animationState;
...
void Armature::setSlotsVisible(bool visible){
    for (size_t i = 0, l = _slotList.size(); i < l; ++i)
    {
        _slotList[i]->updateDisplayVisible(visible);
    }
}
